...
function generateNewPassword() {
  ...
  var addNumbers = confirm(
    "Include numbers in your password? *YES for ok and Cancel for NO*"
  );
  var addUpperCase = confirm(
    "Include upper case letters? *YES for ok and Cancel for NO*"
  );
  var addLowerCase = confirm(
    "Include lower case letters? *YES for ok and Cancel for NO*"
  );
  ...
  for (var i = 0; i < choices.length; i++) {
    var randomLetter =
      choices[i][Math.floor(Math.random() * choices[i].length)];
